...
相手のポケモン名を数文字打つと、自分のパーティ全員の最大打点・確定何発・被弾と、
相手の弱点（×4／×2）がまとめて出る。
...
パーティを短い文字列にして保存・受け渡しできる。`party.html` の「文字列コード」から。
6体で**47文字**。使う文字は英数・ひらがな・カタカナ・常用漢字。

```
C若窒体吸磁舎貴迷右集泊捜胃斤書公曲愉因殊茨遊即碑密矛御乾伏昭奈な塀究該虎梅z宝富然張塚譲社擁
```

`party.txt` を置き換えるものではなく、持ち運び用の別表現。**コメント行は復元されない**
（データとしては同じものに戻る）。コマンドラインからも使える。

```bash
python build/partycode.py                # party.txt をコードにする
python build/partycode.py --decode <コード>
```

コードの意味は `data/code_dict.json`（追記専用の台帳）で決まる。**並べ替えたり
途中に挿入したりしないこと** — 過去に作ったコードが別のポケモンになる。
...
チーム選択画面で相手の6体が見えたら `select.html` に打ち込む。**ひらがなのまま数文字**で
候補が出るので（`がぶ` → ガブリアス）、Enter で確定して次の欄へ進む。相手1体ずつに対して、
...
上段には**相性表**が出る。相手6体と自分の駒を同じ表に並べて、タイプ相性（18タイプの被ダメ倍率）・
無補正の種族値・特性・物理/特殊耐久を一目で見るためのもの。倍率は特性を反映した値で、
特性で変わったところは括弧付き（ふゆうなら じめん が `(0)`）。
この画面だけタイプを1文字表記にしている（ノ炎水電草氷格毒地飛超虫岩霊竜悪鋼妖）。
18列あるので正式名だと横スクロールが要るが、1文字なら1280pxの画面に全部収まる。
**メガ／非メガは相手も自分も切り替えられる**（どちらで見るかは使う人が決める）。
**候補はチャンピオンズ実装済の全313体**で、使用率100位より下の相手も相性表には出る
（処理判定には型・技・持ち物のデータが要るので、そちらだけ出せない）。
3体の組み合わせを全通り試して、
