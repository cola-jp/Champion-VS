...
# 撃つと次のターンに交代できない技。判定が同じなら他の技を優先表示し、
# 打ち合いの必要ターン数も n*2-1 になる。
# **技名を並べないこと。** 一撃必殺・連続技・必ず急所と同じく効果欄から導く。
# 名前で持っていたときは がんせきほう が漏れていて、反動が計算に入っていなかった。
DRAWBACK_MOVES = {name for name, m in MOVES.items()
                  if '次のターン動けない' in (m['effect'] or '')}
...
CONTACT_MOVES = {'アームハンマー', 'ボディプレス', 'サイコファング',
                 ...
                 'ウェーブタックル', 'もろはのずつき', 'おはかまいり', 'せいなるつるぎ',
                 'アクアカッター', 'シェルブレード', 'ダブルウイング',
                 # --- M-C で環境に入った分 ---
                 'きゅうけつ', 'であいがしら', 'アクセルロック', 'グラススライダー',
                 'ドラムアタック', 'きょけんとつげき', 'かえんボール', 'でんこうそうげき',
                 'スターアサルト', 'しっぺがえし',
                 'こおりのキバ', 'ぶんまわす', 'じだんだ', 'ともえなげ'}
# メガランチャー（波動技1.5倍）が乗る技。きれあじ・てつのこぶしと同じく技ごとに書く。
# 効果欄に「波動技」と書く手もあるが、いま書いてあるのは だいちのはどう だけなので、
# 全部の技に書き足すまでは名前で持つ。**名前が「〜のはどう」で終わるとは限らない**
# （はどうだん）ので、規則で決め打ちしないこと。
PULSE_MOVES = {'あくのはどう', 'みずのはどう', 'りゅうのはどう', 'はどうだん',
               'だいちのはどう', 'こんげんのはどう'}

# てつのこぶし（パンチ技1.2倍）が乗る技
PUNCH_MOVES = {'バレットパンチ', 'かみなりパンチ', 'れいとうパンチ', 'ほのおのパンチ',
               'ドレインパンチ', 'コメットパンチ', 'マッハパンチ', 'グロウパンチ',
               'ばくれつパンチ', 'シャドーパンチ', 'メガトンパンチ', 'いじげんラッシュ',
               'ジェットパンチ', 'すいりゅうれんだ', 'アイスハンマー'}
...
NON_CONTACT_MOVES = {'じしん', 'いわなだれ', 'がんせきふうじ', 'ロックブラスト',
                     ...
                     # --- M-C で環境に入った分 ---
                     # ドリルライナー は「ドリルで突く」ので接触に見えるが非接触。
                     # がんせきアックス も岩を飛ばす技で非接触。規則で決め打ちしないこと。
                     'ドリルライナー', 'がんせきアックス', '10まんばりき', 'だいちのはどう',
                     'エアカッター', 'オーバードライブ', 'ねらいうち', 'ハイドロカノン',
                     'もえつきる',
                     # ストーンエッジ は岩を飛ばす技で非接触。名前で決め打ちしない
                     'ストーンエッジ', 'じんつうりき', 'アイアンローラー'}
...
# 表に載せる相手の順位の上限。
# ここを上げると計算対象が増えるだけでなく、図鑑・技・特性・持ち物の未登録が
# 一気に表面化する（ビルドが名前を挙げて止まる／警告する）。上げたら必ず
# python build/generate.py を通してから使うこと。
THREAT_RANK_LIMIT = 150
...
def _parse_party(text):
    ...
    for bi, lines in enumerate(blocks, 1):
        ...
        # メガが2種類ある種（ガブリアス・ルカリオ・アブソル）は持ち物の末尾で見分ける。
        # 「ガブリアスナイトZ」→ メガガブリアスZ、「ガブリアスナイト」→ メガガブリアス。
        # リザードンの X / Y も同じ形（「リザードナイトY」→ メガリザードンY）。
        suffix = item[-1] if item[-1:] in ('X', 'Y', 'Z') else ''
        mega_name = 'メガ' + name + suffix
        if mega_name not in DEX:
            mega_name = 'メガ' + name
